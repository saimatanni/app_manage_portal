...
export default function Residual() {
  ...
  function getPreviousMonth(actualMonth, day) {
    ...
    if (actualMonth === 0 && dayOfMonth < 10) {
      return months[10];
    } else if (actualMonth === 1 && dayOfMonth <= 10) {
      return months[11];
    } else if (actualMonth === 0 && dayOfMonth >= 10) {
      return months[11];
    } else if (dayOfMonth < 10 && actualMonth > 1) {
      return months[actualMonth - 2];
    } else if (dayOfMonth > 10 && actualMonth > 1) {
      return months[actualMonth - 1];
    } else if (dayOfMonth === 10 && actualMonth === 0) {
      return months[11];
    } else return months[actualMonth - 1];
  }

  console.log(currentMonth, "fm 23");
  console.log(getPreviousMonth(currentMonth, currentDate), "fm 2 186");
  ...
  function getYear() {
    ...
    if (currentDay === 10 && currentMonth === 1) {
      return currentDate.getFullYear() - 1;
    }
    if (currentDay > 10 && currentMonth === 1) {
      return currentDate.getFullYear() - 1;
  ...
  console.log(getPreviousMonth(currentMonth, currentDate), "34");
  const [year, setYear] = useState(getYear());
  const [month, setMonth] = useState(
    getPreviousMonth(currentMonth, currentDate)
  );
  ...
  const getChartDataFull = () => {
    ...
    // ptsave_statementdate
    // createdon - "createdon": "2024-01-05T19:05:01Z",
    // "ptsave_statementdate": "2023-12-31T00:00:00Z",
    const currentYear = new Date(
      upfronts[0]?.ptsave_statementdate
    ).getUTCFullYear();
  ...
  useEffect(() => {
    ...
    const chartData = getChartDataFull();
  ...
  useEffect(() => {
    ...
    console.log(upfrontsChart, "515 ");
    console.log(chartData3, "515 ch");
  }, [upfronts, month, year, itemPerPage]);
      ...
      if (sort === "ASC") {
        const param =
          userData?.user_role === 2
            ...
            : "ptsave_partnerresidualamountcalculated";
  ...
  //   value === 0 ? "" : value
  // );
  const filteredChartData = getChartDataFull().map((value) =>
    value === 0 ? "" : value
  );
